...
export default function Dashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [recomendedProducts, lastChanceProducts, recentProducts] =
          await Promise.all([
            ProductService.getFilteredProducts(filters.toString()),
        ...
        {activeFilters && (
          <div className="px-4 py-2 bg-green-50 flex items-center justify-between">
            <div className="text-sm text-green-800">
              Filtres actifs (
              {Object.values(activeFilters.dietary).filter(Boolean).length +
                Object.values(activeFilters.pickupTimes).filter(Boolean)
                  .length +
